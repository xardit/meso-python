...
Përveç numrave, Python gjithashtu mund të manipulojë vargjet, të cilat mund të jenë
shprehur në disa mënyra. Ato mund të mbyllen në thonjëza teke ('...')
ose thonjëza dyshe ("...") me të njëjtin rezultat.
"""
...
def test_string_type():
    """Llojet e vargjeve/Strings"""

    # Varg me thonjëza dyshe
    name_1 = "John"
    # Varg me thonjëza teke.
    name_2 = 'John'
    # Vargjet e krijuara me lloje të ndryshme thonjëzash trajtohen njësoj.
    assert name_1 == name_2
    assert isinstance(name_1, str)
    assert isinstance(name_2, str)

    # \ mund të përdoret për t'i shpëtuar kuotave.
    # përdor \' për t'i shpëtuar thonjëzës së vetme ose për të përdorur thonjëza të dyfishta në vend të tyre.
    single_quote_string = 'doesn\'t'
    double_quote_string = "doesn't"

    assert single_quote_string == double_quote_string

    # \n do të thotë rresht i ri
    multiline_string = 'Rreshti parë.\nRreshti dytë.'
    # Pa print(), \n përfshihet në dalje.
    # Por me print(), \n prodhon një rresht të ri.
    assert multiline_string == 'Rreshti parë.\nRreshti dytë.'

    # Vargjet mund të indeksohen, me karakterin e parë që ka indeksin 0.
    # Vini re se meqenëse -0 është e njëjtë me 0,
    # indekset negative filloni nga -1.
    word = 'Python'
    assert word[0] == 'P'  # Karakteri parë.
    assert word[5] == 'n'  # Karakteri pestë.
    assert word[-1] == 'n'  # Karakteri fundit.
    assert word[-2] == 'o'  # Karakteri parafundit.
    assert word[-6] == 'P'  # Karakteri gjashtë duke filluar nga fundi, ose karakteri zero

    assert isinstance(word[0], str)

    # Përveç indeksimit, suportohet edhe prerja. Ndërsa indeksimi është
    # përdoret për të marrë karaktere individuale, prerja ju lejon të merrni
    # nënvargje:
    assert word[0:2] == 'Py'  # Karakteret nga pozicioni 0 (të përfshirë) në 2 (të përjashtuar).
    assert word[2:5] == 'tho'  # Karakteret nga pozicioni 2 (të përfshirë) në 5 (të përjashtuar).

    # Vini re se si fillimi përfshihet gjithmonë, dhe fundi gjithmonë përjashtohet.
    # Kjo siguron që s[:i] + s[i:] të jetë gjithmonë e barabartë me s:
